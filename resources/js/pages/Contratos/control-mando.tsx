...
const ControlMando = ({ todos_los_departamentos = [], user_rol }: { todos_los_departamentos?: Departamento[], user_rol?: number }) => {
    ...
    const yii_utilidades_url = auth.urls.yii_utilidades_url;
                    ...
                    <table className="w-full text-left border-collapse text-[11px]">
                        <thead>
                            <tr className="bg-gray-100 border-b border-gray-300 uppercase">
                                ...
                                <th className="px-4 py-2 border-r border-gray-300 font-bold text-blue-900 text-center">Fecha Inicio Efectiva</th>
                                <th className="px-4 py-2 border-r border-gray-300 font-bold text-blue-900 text-center">Fecha Fin</th>
                                <th className="px-4 py-2 border-r font-bold text-blue-900 text-center">Estado</th>
                            ...
                            {loading ? (
                                ...
                                <tr key={contrato.id} className="border-b border-gray-200 hover:bg-blue-50/50 transition-colors">
                                    ...
                                    <td className="px-4 py-2 border-r border-gray-200 text-center text-gray-600">
                                        {contrato.fecha_inicio_f ? contrato.fecha_inicio_f : '---'}
                                    </td>
                                    <td className="px-4 py-2 border-r border-gray-200 text-center text-gray-600">
                                        {contrato.fecha_fin_f ? contrato.fecha_fin_f : '---'}
                                    </td>
                                    ...
                                    <td className="px-2 py-2">
                                            <div className="flex flex-col gap-1 items-center">
                                                {!contrato.formalizado && (
                                                    <Link
                                                    href={`/contratos/formalizar/${contrato.id}`} className="w-full min-w-37.5 bg-[#4fc3f7] hover:bg-blue-400 text-white text-[9px] py-0.5 text-center font-bold rounded shadow-sm"
                                                    method='put'
                                                    onClick={() => confirm('¿Está el contrato listo para ser formalizado?')}>
                                                        COMPLETAR CONTRATO
                                                    </Link>)}
                                            ...
                                            <div className="flex gap-1 w-full min-w-37.5">
                                                ...
                                                <Link
                                                    ...
                                                    method="delete"
                                                    as="button"
                                                    onBefore={() => confirm('Dar de baja el contrato?')}
                                                ...
                                                <Link href={`/contratos/edit/${contrato.id}`} className="w-full min-w-37.5 bg-[#4ff7d3] hover:bg-blue-400 text-white text-[9px] py-0.5 text-center font-bold rounded shadow-sm">
                                                        EDITAR CONTRATO
                                                    </Link>
